<template>
  <div class="login-bg">
    <Particles id="login-particles" :options="particlesOptions" class="particles-bg" />
    <div class="login-glow"></div>
    <div class="login-container">
      <div class="brand-title">ClamGuard</div>
      <div class="brand-slogan">{{ $t('login.slogan') }}</div>
      <a-card class="glass-card" style="max-width: 380px; margin: 0 auto;">
        ...
        <h2 style="text-align:center; margin-bottom: 24px; color:var(--brand)">{{ $t('login.title') }}</h2>
        <a-form :model="form" layout="vertical" @finish="onFinish">
          ...
          <a-form-item>
            <a-button type="primary" html-type="submit" block size="large" class="glow-btn">{{ $t('login.login') }}</a-button>
          </a-form-item>
          <div style="text-align:right;">
            <a @click.prevent="goRegister">{{ $t('login.toRegister') }}</a>
          </div>
        </a-form>
      </a-card>
    </div>
